...
feature = np.load("./0001_American_Beauty_00.00.51.926-00.00.54.129.npy")
...
"""
V_p_size = feature.shape[1]
V_t_size = feature.shape[1]
...
for view in range(views):
    ...
    #for v_num, (videos_src, video_ids) in enumerate(feature):
        ...
        #videos = videos_src2 / torch.sqrt(torch.unsqueeze(torch.sum(videos_src2**2, axis=0), 1))
        #sims = torch.mm(videos, videos.t())

        videos_src2 = videos_src / np.sqrt(sum(videos_src**2))
        sims = np.dot(videos_src2, videos_src2.T)
        sim_max = videos_src2.max()
        sim_min = videos_src2.min()
        ...
        videos = torch.from_numpy(videos_src2)
        ...
        action = select_action(state, v_num)
        for v_num2 , videos_2 in enumerate(feature):
            if v_num2 == v_num+1:
                videos_22 = videos_2 / np.sqrt(sum(videos_2**2))
                videos2 = torch.from_numpy(videos_22)
    ...
    #        target_net.load_state_dict(policy_net.state_dict())  
    """
    sum_reward = sum_reward / (len(feature)-1)
    writer.add_scalar("エピソードごとの報酬の遷移", sum_reward, log)
