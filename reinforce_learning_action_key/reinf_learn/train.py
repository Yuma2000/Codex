...
feats_path = "../dataset_toolkit/feats/"
video_name = "BG_335"
# video_name = "BG_22598"
# video_name = "BG_35841"
...
target_name = "target_DQN_"+ video_name + ".pth"
policy_name = "policy_DQN_"+ video_name + ".pth"
model_t_path = os.path.join(model_save, target_name)
model_p_path = os.path.join(model_save, policy_name)

# now = datetime.datetime.now()
dir_name = "log_tensorboard/" + video_name + "_" + now.strftime('%Y%m%d_%H%M%S')
txt_file = "log_text/" + video_name + "_" + now.strftime('%Y%m%d_%H%M%S') + ".txt"
txt_file_max = "log_text/" + video_name + "_" + "max"+ ".txt"
...
if save_frame == True:
    ...
    for s_frame in range(0,int(cap.get(cv2.CAP_PROP_FRAME_COUNT)), 10):
        cap.set(cv2.CAP_PROP_POS_FRAMES, s_frame)
        ret, frame = cap.read()
        save_path = "./save_frame/" + video_name + "/" + str(s_frame) + ".png"
