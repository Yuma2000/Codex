...
feats_path = "./Features/HalfKeyVideos_val/"
video_name = args[1]
...
dtype = torch.float
trainorval = "train"
csv_file1 = "text_file/2LSTM_2021ver/" + video_name +"_2LSTM_"+ trainorval+ "_key.csv"
csv_file2 = "text_file/2LSTM_2021ver/" + video_name +"_2LSTM_"+ trainorval+"_final_key" + ".csv"
#csv_file3 = "text_file/2LSTM_2021ver/" + video_name +"_2LSTM_"+ trainorval+"_best_action" + ".csv"

with open(csv_file1, "w") as f1:
    writer = csv.writer(f1)
    data = ["Video","Frame","Action","Reward"]
...
MODEL_PATH1 = './LSTM_model/LSTM_AutoEncoder_2LSTM_Epoch_10000_lr_1e-5_half_train_2021ver.pth'
...
for i1, (videos1_src, video_ids1) in enumerate(feature1):
    ...
    videos1_src = videos1_src.to(device)
    videos1_src = videos1_src.reshape((videos1_src.shape[1], videos1_src.shape[2]))
    videos1_src = videos1_src / torch.sqrt(torch.unsqueeze(torch.sum(videos1_src**2, axis=1),1))
    sims = torch.mm(videos1_src, videos1_src.t())
    sims.fill_diagonal_(-100)
    sim_max = sims.max()
    sims.fill_diagonal_(100)
    sim_min = sims.min()
    videos1 = torch.unsqueeze(videos1_src, 0)
    optimizer.zero_grad()
...
for i3, (videos3_src, video_ids3) in enumerate(feature3):
    ...
    videos3_src = videos3_src.to(device)
    videos3_src = videos3_src.reshape((videos3_src.shape[1], videos3_src.shape[2]))
    videos3_src = videos3_src / torch.sqrt(torch.unsqueeze(torch.sum(videos3_src**2, axis=1),1))
    sims = torch.mm(videos3_src, videos3_src.t())
    sims.fill_diagonal_(-100)
    sim_max = sims.max()
    sims.fill_diagonal_(100)
    sim_min = sims.min()
    videos3 = torch.unsqueeze(videos3_src, 0)
...
for i, (videos_src, video_ids) in enumerate(feature):
    ...
    videos_src = videos_src.reshape((videos_src.shape[1], videos_src.shape[2]))
    videos_src = videos_src / torch.sqrt(torch.unsqueeze(torch.sum(videos_src**2, axis=1),1))
    sims = torch.mm(videos_src, videos_src.t())
    sims.fill_diagonal_(-100)
    sim_max = sims.max()
    sims.fill_diagonal_(100)
    sim_min = sims.min()
    videos = torch.unsqueeze(videos_src, 0)
    ...
    for frame in range(max_frame-1):
        ...
        """
        # stack_reward += reward
        stack_reward.append(reward.item())
        data = [str(i),str(frame),str(action.item()),str(reward.item())]
        data_list.append(data)
        #text1 = str(i) + "," + str(frame) + "," + str(action.item()) + "," + str(reward.item()) + "\n"
        #f1.writelines(text1)
        #text3 = str(i) + "," + str(frame) + "," + str(action.item()) + "\n"
        #f3.writelines(text3)
...
with open(csv_file2, "a") as f2:
    data = ["final",str(np.mean(stack_reward))]
    writer = csv.writer(f2)
    writer.writerow(data)

print(np.mean(stack_reward))
