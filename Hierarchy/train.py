...
from args import train_range, train_prediction_txt_path, train_reference_txt_path
...
#reference_json_path = '{0}.json'.format(val_reference_txt_path)
#reference_json_path = '{0}.json'.format(train_reference_txt_path)
...
for epoch in range(num_epochs):
    ...
    for i, (videos, captions, cap_lens, video_ids) in enumerate(train_loader, start=1):
        ...
        #videos = videos.clone().requires_grad_(True)
        videos = videos.float().requires_grad_(True)
        targets = captions.long()
        ...
        #outputs, video_encoded = banet(vid, targets, epsilon)
    ...
    print(test_prediction_txt_path)
    #metrics = evaluate(vocab, banet, val_range, val_prediction_txt_path, reference)
    #metrics = evaluate(vocab, banet, train_range, train_prediction_txt_path, reference)
